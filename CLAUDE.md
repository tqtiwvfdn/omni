...
1. 前端样式只能使用 tailwindCSS 和 /style/common.css 的公共样式,禁止在页面内写<style>、行内样式，自定义样式包括变量和自定义样式类名。
  包括：左侧菜单、右侧内容、列表、表格、按钮、表单、表单项（input、select、radio、range、checkbox），等元素；
  每个自定义类名都需要绑定变量
  我希望有暗色和明色两种变量；
2. 前端每个页面使用一个原型对象实现，并把实例对象暴露到window下。禁止使用过度封装的方法、但也不能使用毫无封装的内容，选择器智能使用id、data-role选择器，不能使用其他选择器。
  每个按钮操作都能在这个对象实例找到方法；